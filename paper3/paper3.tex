...
Many diverse applications in the system for infrastructure provider to select and serve. In our model, we choose some of applications as $I$ to allocate network slice.   $i\in I, I_{i} = \{F, t\}$, where $F$ is the file size of application need to be computed and transferred, $t$ is the maximum delay required by application, the delay contains computing time and transferring time. When an application $i$ has been finished within the delay successfully, it can provide a profit $p_{i}$. Next, we derive our model.



\begin{align}
maximize\quad &\sum_{i=1}^{I} p_{i} R_{i} \\
